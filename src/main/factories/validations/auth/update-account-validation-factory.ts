...
export const makeUpdateAccountValidation = ({ minLengthName, minLengthPassword, emailValidator }: UpdateAccountValidationConfig): ValidationComposite => {
  ...
  const passwordValidation = ValidationBuilder.field('password').min(minLengthPassword).conditionalRequired('new_password')
  const newPasswordValidation = ValidationBuilder.field('new_password').min(minLengthPassword).conditionalRequired('password')
